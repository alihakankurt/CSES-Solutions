...
int main()
{
    ...
    auto values = vector<i64>(valuesLength + 1u);
    for (usize position = 1u; position <= valuesLength; position += 1u)
    {
        cin >> values[position];
    }

    auto diff = BinaryIndexedTree<i64>(valuesLength + 1u);
    ...
    for (usize _ = 1u; _ <= queriesLength; _ += 1u)
    {
        ...
        if (type == 1u)
        {
            ...
            diff.Update(left, value);
            diff.Update(right + 1, -value);
        }
        else if (type == 2u)
        {
            usize position;
            cin >> position;
            i64 answer = values[position] + diff.Query(position);
            cout << answer << '\n';
        }
    }
